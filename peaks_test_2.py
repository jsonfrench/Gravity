...
ratio_peaks, _ = find_peaks(ratio_vals, height = (np.nanmin(ratio_vals)) + (0.5*(np.nanmax(ratio_vals) - np.nanmin(ratio_vals))))
peak_time = times[ratio_peaks]
peak_height = ratio_vals[ratio_peaks]
ax_ratio_twin.scatter(peak_time, peak_height)

# print(f"height range: {(np.nanmax(ratio_vals) - np.nanmin(ratio_vals))}")
# print(f"height max: {np.nanmax(ratio_vals)} height min: {np.nanmin(ratio_vals)}")
print(f"Outer body mass: {m2}")
print(f"Average Peak Y value: {np.mean(peak_height)}")
print(f"Average Peak height: {np.mean(peak_height - np.nanmin(ratio_vals))}")
