...
mass_earth = mass_earth / mass_sun
mass_mars = mass_mars / mass_sun
mass_sun = 1
...
v_mars = np.sqrt(G * mass_sun / r_mars) * v_factor
...
def finite_diff_accel2(pos, dt):
    acc = np.zeros_like(pos)
    for k in range(4, len(pos)):
        acc[k-2] = (pos[k] - 2*pos[k-2] + pos[k-4]) / (4 * dt**2)
    return acc
...
def compute_plot_limits(R, ecc=0.0):
    return 2 * np.max(R) / (1 - ecc**2)

# accel_earth_approx = finite_diff_accel(positions[1], dt)
accel_earth_approx = finite_diff_accel2(positions[1], dt)
...
lim = compute_plot_limits([r_earth,r_mars], ecc=1-1/v_factor)
...
plots.plot_local_synodic_radius_markers()
