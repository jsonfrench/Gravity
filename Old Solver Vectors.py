...
Fx_sun = -G * M * m1 * x1s / r1s**3
Fy_sun = -G * M * m1 * y1s / r1s**3

Fx_mars = (G * m2 * (x2s - x1s) / ds**3) * m1
Fy_mars = (G * m2 * (y2s - y1s) / ds**3) * m1
...
Fsun_vector = None
Fmars_vector = None
Fnet_calculated_vector = None
Fmars_calculated_vector = None
...
def update(val):
    ...
    global Fsun_vector
    global Fmars_vector
    global Fnet_calculated_vector
    global Fmars_calculated_vector
    ...
    #Remove previous Fnet vector
       #Remove previous Fnet vector
    if Fmars_calculated_vector is not None:
        Fmars_calculated_vector.remove()

    if Fmars_vector is not None:
        Fmars_vector.remove()
    
    if Fsun_vector is not None:
        Fsun_vector.remove()
    
    if Fnet_vector is not None:
        Fnet_vector.remove()
    
    if Fnet_calculated_vector is not None:
        Fnet_calculated_vector.remove()

    #Update Force Vector
    Fsun_vector = ax_orbit.quiver(x1s[idx], y1s[idx], Fx_sun[idx], Fy_sun[idx], color='yellow', scale=1e-10)
    Fnet_vector = ax_orbit.quiver(x1s[idx], y1s[idx], m1*ax1[idx], m1*ay1[idx], color='orange', scale=1e-10)
    Fmars_vector = ax_orbit.quiver(x1s[idx], y1s[idx], (ax1[idx] - axE[idx]) * m1, (ay1[idx] - ayE[idx]) * m1, color='r', scale=1e-10)
    Fnet_calculated_vector = ax_orbit.quiver(x1s[idx], y1s[idx], (Fx_mars[idx] + Fx_sun[idx]) / Scale_factor, (Fy_mars[idx] + Fy_sun[idx]) / Scale_factor, color='orange', scale=1e-10)
    Fmars_calculated_vector = ax_orbit.quiver(x1s[idx], y1s[idx], Fx_mars[idx], Fy_mars[idx], color='purple', scale=1e-10)
